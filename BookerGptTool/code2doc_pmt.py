...
从代码中提取给定函数或方法{func}，输出名称（原样重复）、参数名称、参数类型、参数描述、返回值类型、返回值描述、mermaid 流程图、和带注释源码。
...
我们已经包含了一段话概述、整体流程、类结构、类字段、类方法、全局变量、全局函数信息、关键组件信息、问题及改进建议。请列举详细设计文档还应该包含什么项目（如设计目标与约束、错误处理与异常设计、数据流与状态机、外部依赖与接口契约等等）。注意只需要输出格式中包含的内容，不要输出任何其它东西。
