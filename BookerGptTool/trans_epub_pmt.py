...
+   主干层级：只认全书最高层级的大章（如“第一章”、“第二部分”）。如果候选是“1.1”、“第一节”等子层级，直接剔除。注意也可能没有这种序号，这就需要依靠语义判断。
+   格式特征：真正的章标题通常单独成行，且前后在Markdown源码中有空行；标题末尾绝无句号（。.？！），且长度通常小于30字。
+   上下文语义：标题的后10行通常是对该章节的宏观概述，而非紧接具体的对话或人物动作。如果后10行出现“他说：“”或具体时间地点，则判定为正文误判，予以剔除。
+   序号连贯性：如果候选序号明显跳跃（如只有第1章、第3章，缺第2章），只保留序号连贯且符合全书逻辑的部分。
