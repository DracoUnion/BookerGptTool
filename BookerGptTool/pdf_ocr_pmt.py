...
MERGE_PMT = '''
你是一个专业的文档编辑助手。你将会得到两段 Markdown 文本，分别位于第一页最后一行和第二页第一行，请判断它们是否属于同一段落。如果第一段文本没有结束（例如：缺少句号、引号，或者语义明显未完），请将其与下一页的开头合并为同一段落。 如果句子已经结束，则保留为独立段落。如果属于同一段落，输出 [TRUE]，否则为 [FALSE]，不要输出其它选项。
...
- 如果只有图片链接，那么仅仅保留并输出图片链接。
- 如果没有任何有效内容。输出“[content]<!-- no content -->[/content]”。
