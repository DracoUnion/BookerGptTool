...
class Code2BookOrchestrator(Code2BookMixin):
    ...
    def _tr_gen_detail(
        ...
        detail = Detail(no=idx+1, **src_anls_result.dict(), **rest_result.dict())
...
def reg_subparser(subparsers):
    code2book_parser = subparsers.add_parser("code2book", help="代码 → 书籍")
    code2book_parser.add_argument('dir', help='项目目录名')
    code2book_parser.add_argument("-t", "--threads", type=int, default=8, help="线程数")
    code2book_parser.add_argument("-c", "--check", type=int, default=3, help="校验次数")
    code2book_parser.add_argument("-l", "--chapter-limit", type=int, default=20, help="章节数上限")
    code2book_parser.add_argument("-D", "--debug", action='store_true', help="调试模式")
    code2book_parser.add_argument("-cl", "--code-limit", type=int, default=35_000, help="单次提示中代码长度上限")
    code2book_parser.add_argument("-co", "--code-overlap", type=int, default=500, help="每个分块的代码重叠量")
    code2book_parser.set_defaults(func=code2book)

    code2book_parser = subparsers.add_parser("code2book-check", help="代码 → 书籍校验器")
    code2book_parser.add_argument('dir', help='以 _code2book 结尾的项目目录名')
    code2book_parser.add_argument("-t", "--threads", type=int, default=8, help="线程数")
    code2book_parser.add_argument("-c", "--check", type=int, default=3, help="校验次数")
    code2book_parser.add_argument("-D", "--debug", action='store_true', help="调试模式")
    code2book_parser.add_argument("-co", "--check-outline", action='store_true', help="是否校验大纲")
    code2book_parser.add_argument("-cd", "--check-detail", action='store_true', help="是否校验详情")
    code2book_parser.add_argument("-cb", "--check-body", action='store_true', help="是否校验正文")
